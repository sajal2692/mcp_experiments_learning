...
class MCP_ChatBot_All:
    ...
    async def connect_to_local_server(self, server_name, server_config):
        ...
        try:
            ...
            print(f"Error connecting to local server {server_name}: {e}")
    ...
    async def connect_to_remote_server(self, server_name, server_url):
        ...
        try:
            ...
            print(f"Error connecting to remote server {server_name}: {e}")

    async def _register_server_capabilities(self, session, server_name):
        """Register tools, prompts, and resources from a server session."""
        try:
            ...
            print(f"Error registering capabilities from {server_name}: {e}")
    ...
    async def connect_to_all_servers(self):
        ...
        try:
            ...
            for server_name, server_config in local_servers.items():
                await self.connect_to_local_server(server_name, server_config)
        except Exception as e:
            print(f"Error loading local server config: {e}")

        # Connect to remote servers
        try:
            with open("remote_server_config.json", "r") as file:
                remote_data = json.load(file)
            remote_servers = remote_data.get("remoteMcpServers", {})
    ...
    async def chat_loop(self):
        print("\nUnified MCP Chatbot Started!")
        print("Connected to both local and remote MCP servers")
